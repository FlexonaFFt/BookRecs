...
- **Почему использование ML улучшит текущее решение?**
  Проекты, которые используют рекомендации без ML обычно используют подход (популярное / новинки) и имеет рад минусов: одинаковые рекомендации для всех пользователей, не учитывает личные предпочтения, не показывает популярные или релевантные произведения для пользователя.
...
- **Что закрываем в данной итерации (DS)**
  ...
  В этой итерации мы собираем базовую, но рабочую систему рекомендаций:
