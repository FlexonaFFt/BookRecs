...
- **Разделение учебного и продуктового контуров**
  ...
  - в рамках продуктового контура (пилот/production) дополнительно предполагаются `impression`-логи, A/B-инфраструктура, данные о наличии и технические метрики сервиса;
...
- Учитываем два режима:
  - **warm-книги**: у книги есть не меньше `k` взаимодействий до даты разделения (`k` фиксируем в эксперименте, например `k=5`);
...
- Для воспроизводимости заранее фиксируем схему весов implicit feedback (пример для первой версии): `view=1`, `click=2`, `add_to_cart=4`, `purchase=6` (дальше можно настраивать, но не менять между сравниваемыми моделями).
...
- Метрики:
  ...
  - дополнительно для контроля перекоса в бестселлеры: `CatalogCoverage`, доля показов long-tail книг (или аналогичная proxy-метрика разнообразия).
- Базовый вариант:
