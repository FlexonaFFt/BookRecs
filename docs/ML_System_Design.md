...
B --> C1["Признаки пользователя<br/>(интересы, недавняя активность)"]
B --> C2["Признаки книги<br/>(контент и метаданные)"]

C1 --> D["Гибридная модель<br/>(поведение + контент)"]
C2 --> D

D --> E["Ранжированный список Top-10"]

E --> F["Бизнес-фильтры<br/>(например, доступность)"]

F --> G["Оценка качества<br/>(warm / cold сегменты)"]
...
| --- | --- | --- | --- |
| История взаимодействий пользователя (Goodreads YA: `is_read`, `rating`, `date_added`) | датасет Goodreads YA | обучение и офлайн-оценка (warm/cold) | пропуски, дубли, корректность временных полей, валидность диапазона `rating` |
| Метаданные книг (жанр, автор, серия, описание, язык, год) | Goodreads YA metadata / каталог книги | контентные признаки в гибридной модели, особенно для cold-item | заполненность ключевых полей, нормализация категорий, доля редких/шумных значений |
| Служебные признаки эксперимента (`is_cold_item`, границы train/valid/test) | формируем в preprocessing | честное сравнение моделей и отдельный cold-срез | отсутствие leakage из будущего, стабильность порога `k`, воспроизводимость split |
| Событийные логи рекомендаций (`impression/click/...`) [roadmap, не в текущем MVP] | продуктовые логи (после внедрения) | онлайн A/B-оценка и расчет продуктовых метрик | полнота логов, корректная атрибуция пользователю/сессии, контроль `SRM` |
...
- Основные статьи затрат в месяц:
  - вычисления для переобучения модели (обычно low-medium);
  - онлайн-инференс (обработка запросов рекомендаций; обычно medium);
  - хранение логов и артефактов моделей (обычно low-medium);
  - поддержка мониторинга и алертов (обычно low).
